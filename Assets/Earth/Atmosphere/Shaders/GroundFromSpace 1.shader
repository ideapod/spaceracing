...
Shader "Atmosphere/GroundFromSpaceVertex" 
{
	Properties {
		_MainTex ("Base (RGB)", 2D) = "white" {}
		_BumpMap ("Bumpmap", 2D) = "bump" {}
		_Emmision ("Emmision", Range(1.0,5.0)) = 2.0
	}
	SubShader 
	{
    	...
    	Pass 
    	{
			...
			half4 frag(v2f IN) : COLOR
			{
				...
				half3 pNormal = UnpackNormal( tex2D(_BumpMap, IN.uv) );
				
				float3 col = IN.c0 + 0.15 * IN.c1;
				//Adjust color from HDR
				col = 1.0f - exp(col * -fHdrExposure);
				texel *= col.b * _Emmision;
				return half4(texel + col + (pNormal * 0.15),1.0);
			}
