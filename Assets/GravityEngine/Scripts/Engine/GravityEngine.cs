...
public class GravityEngine : MonoBehaviour {
    ...
    private PatchedConicSOI[] patchedConicSOIs; 
	...
	public void Setup() {
        ...
        patchedConicSOIs = (PatchedConicSOI[])Object.FindObjectsOfType(typeof(PatchedConicSOI));
...
#pragma warning restore 162       // disable unreachable code warning
        if (evolve) {
            ...
            // not awesome to check for input events in Fixed Update, but hey, debug
            ...
            lastWorldDt = Time.fixedDeltaTime * timeZoom;
...
#pragma warning restore 162       // disable unreachable code warning
    ...
    private void EvolveToTime(double toTime) {

        double gameDt = toTime; 

        if (trajectoryPrediction) {
            EvolveTrajectory(gameDt);
        }

        // Maneuver processing - if there is a manuever in the next interval, try to evolve as close to its target time
        // as possible, then finish the time step

        trajectoryRestart = worldState.Evolve(this, gameDt);
        ...
        // patched conics
        // updated per fixed update (so closest approach is subject to this time quantization)
        // Would need to move inside integrator for maximum precision. 
        if (patchedConicSOIs != null) {
            foreach (PatchedConicSOI p in patchedConicSOIs) {
                p.GEUpdate();
            }
        }
    ...
    public void FastForward(float time) {
        EvolveToTime(worldState.time + time);
    }
	...
	private void SetupOneGameObject(GameObject gameObject, NBody nbody) {
        ...
        if (fixedOrbit != null && fixedOrbit.IsFixed()) {
            ...
            FixedBody fixedBody = new FixedBody(worldState.numBodies, fixedOrbit);

            OrbitEllipse oe = gameObject.GetComponent<OrbitEllipse>();
            if (oe != null) {
                fixedBody.kepler_depth = oe.CalcKeplerDepth();
            }
		...
		if (nbody.mass == 0 && optimizeMassless && !fixedObject) {
            ...
            #pragma warning disable 162       // disable unreachable code warning
            if (worldState.masslessEngine == null) {
                worldState.masslessEngine = new MasslessBodyEngine(engineDt);
            ...
            worldState.masslessEngine.AddBody(gameObject, physToWorldFactor);
			...
			worldState.m[worldState.numBodies] = nbody.mass * massScale;
			...
			worldState.r[worldState.numBodies, 0] = physicsPosition.x;
			worldState.r[worldState.numBodies, 1] = physicsPosition.y;
			worldState.r[worldState.numBodies, 2] = physicsPosition.z; 
			worldState.size2[worldState.numBodies] = nbody.size * nbody.size;
			gameObjects[worldState.numBodies] = gameObject;
            worldState.integrator.AddNBody(worldState.numBodies, nbody, physicsPosition, nbody.vel_phys);
	...
	public void RemoveBody(GameObject toRemove) {
        ...
        if ((worldState.info[nbody.engineRef.index] & FIXED_MOTION) != 0) {
			...
			foreach(FixedBody fb in worldState.fixedBodies) {
			...
			if (fbRemove != null) {
				worldState.fixedBodies.Remove(fbRemove);
			}
		}
		if (nbody.engineRef.bodyType == BodyType.MASSIVE) {
            ...
            worldState.RemoveBodyAtIndex(nbody.engineRef.index);
        } else {
            worldState.masslessEngine.RemoveBody(toRemove);
		}
		if (trajectoryPrediction) {
			trajectoryRestart = true;
		}
	...
	public void InactivateBody(GameObject toInactivate) {
		...
		if (nbody.engineRef.bodyType == BodyType.MASSLESS ) {
            worldState.masslessEngine.InactivateBody(toInactivate);
		} else {
	...
	public void ActivateBody(GameObject activate) {
		...
		if (nbody.engineRef.bodyType == BodyType.MASSLESS ) {
            worldState.masslessEngine.ActivateBody(activate);
		} else {
			int i = nbody.engineRef.index;
	...
	private void UpdateGameObjects() {
		for (int i=0; i < worldState.numBodies; i++) {
            ...
            // TODO: Crazy to do this every time! Ditch GetComponent
            NBody nbody = gameObjects[i].GetComponent<NBody>();
            if ((worldState.info[i] & INACTIVE) == 0) {
                if ((worldState.info[i] & FIXED_MOTION) == 0) {
                    ...
                    if (NUtils.VectorNaN(position)) {
                        InactivateBody(gameObjects[i]);
                        Debug.LogWarning("Position NaN - inactivated " + gameObjects[i].name);
                    } else {
                        position = physToWorldFactor * position;
                    ...
                    if (nbody.engineRef.fixedBody.fixedOrbit != null) {
                        nbody.engineRef.fixedBody.fixedOrbit.GEUpdate(this);
                    }
                }
            }
		}
    ...
    public void MoveAll(Vector3 move) {
        ...
        foreach (FixedBody fixedBody in worldState.fixedBodies) {
            fixedBody.fixedOrbit.Move(move);
        }
    ...
    public Vector3 GetVelocity(NBody nbody) {
        ...
        if ((nbody.engineRef.fixedBody != null) && (nbody.engineRef.fixedBody.fixedOrbit != null)) {
			...
			return worldState.masslessEngine.GetVelocity(nbody);
		} 
		return worldState.integrator.GetVelocityForIndex(nbody.engineRef.index);
	}
    ...
    public double GetMass(NBody nbody) {
        if (nbody.engineRef == null) {
            Debug.LogWarning("NBody not initialized");
            return double.NaN;
        }
    ...
    /// <param name="body">Body</param>
    public Vector3 GetScaledPosition(NBody body) {
	...
	private Vector3 ApplyImpulseInternal(NBody nbody, Vector3 impulse, bool apply) {
		...
		if (nbody.engineRef.bodyType == BodyType.MASSIVE) {
			...
			velocity = worldState.masslessEngine.GetVelocity(nbody.transform.gameObject);
			velocity += impulse;
			if (apply) {
                worldState.masslessEngine.SetVelocity(nbody.transform.gameObject, velocity);
			}
		}
    ...
    public string GetScaledTimeFormatted() {
        ...
        return GravityScaler.GetWorldTime(worldState.time, units); 
    }
	...
	public string DumpAll(GravityState gs) {
		...
		for (int i=0; i < gs.numBodies; i++) {
			...
			sb.Append(string.Format("   n={0} {1} m={2} r={3} {4} {5} v={6} {7} {8} info={9}  \n", i, gameObjects[i].name, 
				...
				vel.x, vel.y, vel.z, gs.info[i]
					));
		}
        // Title printed by MBE
        if (worldState.masslessEngine != null) {
			sb.Append(worldState.masslessEngine.DumpString());
		}
        sb.Append("Fixed Bodies:\n");
        for (int i=0; i < worldState.fixedBodies.Count; i++) {
            int fb_index = worldState.fixedBodies[i].index;
            sb.Append(string.Format("   n={0} {1} kepler_depth={2}\n", 
                i, 
                gameObjects[fb_index], 
                worldState.fixedBodies[i].kepler_depth));
        }
		return sb.ToString();
	}
    ...
    public class InfoCommand : GEConsole.GEConsoleCommand
    {
        ...
        public string Run(string[] args) {
            ...
            pos = ge.GetScaledPosition(nbody);
