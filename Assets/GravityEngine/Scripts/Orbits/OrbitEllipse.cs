...
public class OrbitEllipse : EllipseBase, INbodyInit, IFixedOrbit, IOrbitScalable {
	...
	public void InitNBody(float physicalScale, float massScale) {
        ...
        if (evolveMode == evolveType.KEPLERS_EQN) {
            if (Application.isPlaying) {
                ...
                Evolve(ge.GetPhysicalTime(), ge.GetLengthScale(), ref r);
            } else {
    ...
    /// <summary>
    /// Determine how many parents/grandparents etc. have OrbitEllipse/Kepler mode. 
    /// GE uses this to ensure evolution starts at the most central body in a heirarchy
    /// and works out to the leaves. 
    /// </summary>
    public int CalcKeplerDepth() {
        if (evolveMode != evolveType.KEPLERS_EQN) {
            return 0;
        }
        int depth = 0;
        bool done = false;
        GameObject center = centerObject;
        while(!done && (center != null)) {
            OrbitEllipse oe = center.GetComponent<OrbitEllipse>();
            if ((oe != null) && (oe.evolveMode == evolveType.KEPLERS_EQN)) {
                depth++;
                center = oe.centerObject;
            } else {
                done = true; 
            }
        }
