...
#pragma warning restore 162
	...
	private void GrowArrays(int growBy) {
        ...
        RocketEngine[] rockets_copy = new RocketEngine[arraySize];
        ...
        rockets = new RocketEngine[arraySize + growBy];
		for( int j=0; j < numBodies; j++) {
			for (int k=0; k < GravityState.NDIM; k++) {
				r[j,k] = r_copy[j, k]; 
				v[j,k] = v_copy[j, k]; 
    ...
    public void MoveTrajectories(Vector3 moveBy) {
        for (int n = 0; n < numBodies; n++) {
